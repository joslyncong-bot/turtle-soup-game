...
export default function App() {
  ...
  const startNewGame = useCallback(async (mode: 'random' | 'dynamic' = 'random') => {
    ...
    if (mode === 'dynamic' && apiKey) {
      ...
      setCurrentStory({ id: Date.now(), riddle: '', truth: '' });
      
      try {
        const ai = new GoogleGenAI({ apiKey });
        const response = await ai.models.generateContentStream({
          model: "gemini-3-flash-preview",
          contents: [{ role: "user", parts: [{ text: "生成一个全新的、高质量的中文海龟汤故事。包含谜面（悬疑感强）和汤底（逻辑严密且出人意料）。请以 JSON 格式返回，包含 riddle 和 truth 字段。不要包含任何 Markdown 标记。" }] }],
          config: {
            // We can't use responseMimeType: "application/json" with streaming easily if we want to show partial text
            // But we can stream the text and parse it at the end.
            temperature: 0.8,
          }
        });
        ...
        for await (const chunk of response) {
          ...
          // Try to extract partial riddle for typewriter effect
          // This is a bit hacky since it's JSON, but we can try to find the riddle value
          const riddleMatch = fullText.match(/"riddle":\s*"([^"]*)"?/);
          if (riddleMatch && riddleMatch[1]) {
            setCurrentStory(prev => prev ? { ...prev, riddle: riddleMatch[1] } : null);
          }
        }
        
        // Final parse
        try {
          // Clean up potential markdown code blocks if AI ignored instructions
  ...
  const handleGetHint = async () => {
    ...
    // Add an empty host message for the hint
    ...
    try {
      ...
      for await (const chunk of response) {
        ...
        setMessages(prev => prev.map(m => 
          m.id === hintId ? { ...m, content: `[提示] ${fullHint}` } : m
        ));
      }
    } catch (error) {
      console.error('Hint Error:', error);
      setMessages(prev => prev.filter(m => m.id !== hintId));
    } finally {
  ...
  const handleSend = async (text: string = input) => {
    ...
    const hostMessageId = (Date.now() + 1).toString();
    setMessages(prev => [...prev, {
      id: hostMessageId,
      role: 'host',
      content: ''
    ...
    try {
      ...
      for await (const chunk of response) {
        ...
        fullReply += chunkText;
        setMessages(prev => prev.map(m => 
          m.id === hostMessageId ? { ...m, content: fullReply } : m
        ));
      }

      const solved = fullReply.includes('恭喜你') || fullReply.includes('真相是');
      if (solved) {
        setIsSolved(true);
      }
      
      checkAchievements(fullReply, solved);
    } catch (error) {
      console.error('AI Error:', error);
      setMessages(prev => prev.map(m => 
        m.id === hostMessageId ? { ...m, content: '主持人似乎陷入了沉思，请稍后再试。' } : m
      ));
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Mysterious Forest Side Backgrounds */}
      <div className="fixed inset-y-0 left-0 w-1/2 opacity-40 pointer-events-none overflow-hidden hidden md:block z-0">
        <img 
          src="https://images.unsplash.com/photo-1441974231531-c6227db76b6e?auto=format&fit=crop&q=80&w=1000" 
          alt="" 
          className="h-full w-full object-cover brightness-50 contrast-125 saturate-50"
          style={{ maskImage: 'linear-gradient(to right, black 10%, transparent 70%)', WebkitMaskImage: 'linear-gradient(to right, black 10%, transparent 70%)' }}
          referrerPolicy="no-referrer"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0a0a0f]/60 via-transparent to-transparent" />
      </div>
      <div className="fixed inset-y-0 right-0 w-1/2 opacity-40 pointer-events-none overflow-hidden hidden md:block z-0">
        <img 
          src="https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&q=80&w=1000" 
          alt="" 
          className="h-full w-full object-cover brightness-50 contrast-125 saturate-50"
          style={{ maskImage: 'linear-gradient(to left, black 10%, transparent 70%)', WebkitMaskImage: 'linear-gradient(to left, black 10%, transparent 70%)' }}
          referrerPolicy="no-referrer"
        />
        <div className="absolute inset-0 bg-gradient-to-l from-[#0a0a0f]/60 via-transparent to-transparent" />
      </div>
